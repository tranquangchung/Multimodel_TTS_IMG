...
class MultiTaskImageSpeech(nn.Module):
    ...
    def _set_image_lora(self, enabled: bool):
    ...
    def speech_forward(
        ...
        t_emb = self.text_embeddings(idx)
        param = next(self.img.parameters())
        h = t_emb.to(device=param.device, dtype=param.dtype)

        seq_len = t_emb.shape[1]
        freqs_cis = self.speech_freqs_cis[:seq_len].to(t_emb.device)
        ...
        # Optional residual from input embeddings
        # if self.residual_connection:
        #     h = h + t_emb  # residual connection
    ...
    def image_generate(
        ...
        self.img.eval().to(dtype=torch.bfloat16)
        self._set_image_lora(False)
