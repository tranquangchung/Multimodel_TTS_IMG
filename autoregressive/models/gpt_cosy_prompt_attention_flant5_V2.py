...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, kv):                     # x: [B,T,D], kv: [B,S,D]
        ...
        k = self.k(kv).view(B,S,H,d).transpose(1,2)
        v = self.v(kv).view(B,S,H,d).transpose(1,2)
...
class TransformerSpeechBlock(nn.Module):
    ...
    def forward(self, x, freqs_cis, start_pos, mask=None, style_embeddings=None, index=None):
        ...
        #     s = self.drop_path(self.cross_attn(self.cross_norm(h), style_embeddings))
        #     h = s

        if style_embeddings is not None:
            # align dtype/device
            kv = style_embeddings.to(h.device, dtype=h.dtype)  # OK
            # chuẩn hoá theo thống kê của h (feature-wise, trung bình theo trục thời gian)
            eps = torch.finfo(h.dtype).eps
            h_norm = self.cross_norm(h)  # [B,T,D]
            # stats của h và kv (theo trục seq: T cho h, S cho kv)
            h_mu = h_norm.mean(dim=1, keepdim=True)  # [B,1,D]
            h_std = h_norm.var(dim=1, unbiased=False, keepdim=True).add(eps).sqrt()
            kv_mu = kv.mean(dim=1, keepdim=True)  # [B,1,D]
            kv_std = kv.var(dim=1, unbiased=False, keepdim=True).add(eps).sqrt()
            # đưa kv về cùng range với h
            kv_aligned = (kv - kv_mu) / kv_std * h_std + h_mu  # [B,S,D]
            s = self.drop_path(self.cross_attn(h_norm, kv_aligned))
            h = h + s
...
class MultiTaskImageSpeech(nn.Module):
    ...
    def speech_forward(
        ...
        h = self.text_embeddings(idx).to(device=p.device, dtype=p.dtype)  # [B,T,D]
        B, T, D = h.shape

        # ROPE cho full sequence sau khi prepend
        seq_len = h.shape[1]
        ...
        if style_embeddings is not None:
            style_embeddings = style_embeddings.to(device=h.device, dtype=h.dtype)  # [B,S,Ds]
            h = torch.cat([style_embeddings, h], dim=1)  # [B,S+T,D]

        # ------ speech-only layers (KHÔNG cross-attn nữa vì đã prepend) ------
        for index, layer in enumerate(self.speech_layers):
            h = layer(h, freqs_cis, input_pos, None, style_embeddings=None, index=index)

        h = self.speech_norm(h)
        logits = self.speech_head(h).float()  # [B,P+T,V]

        # ------ loss: bỏ P token điều kiện, dịch 1 bước AR ------
        loss = None
        if targets is not None:
            # targets vẫn là độ dài T (không có P). Căn theo logits đã prepend:
            logits_shift = logits[:, P:-1, :].contiguous()  # [B, (P+T-1)-P, V] = [B, T-1, V]
            targets_shift = targets[:, :logits_shift.size(1)].contiguous()  # [B, T-1]
            loss = F.cross_entropy(
